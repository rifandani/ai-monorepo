...
import { ContextualRecallJudge } from '@/evalite/llm/contextual-recall/metricJudge';
...
export class ContextualRecallMetric extends Metric {
  ...
  constructor(
    model: LanguageModel,
    { scale = 1, context }: ContextualRecallMetricOptions
  ) {
    super();
  ...
  async measure(
    ...
    const reason = await this.judge.getReason({
      ...
      supportiveReasons: verdicts
        .filter((v) => v.verdict === 'yes')
        .map((v) => v.reason),
      unsupportiveReasons: verdicts
