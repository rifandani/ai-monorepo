...
import { PromptAlignmentJudge } from '@/evalite/llm/prompt-alignment/metricJudge';
...
export class PromptAlignmentMetric extends Metric {
  ...
  constructor(
    model: LanguageModel,
    { instructions, scale = 1 }: PromptAlignmentMetricOptions
  ) {
    super();
  ...
  async measure(
    input: string,
    output: string
  ): Promise<PromptAlignmentMetricResult> {
    const verdicts = await this.judge.evaluate(
      input,
      output,
      this.instructions
    );
  ...
  private calculateScore(
    ...
    const score =
      counts.applicableCount > 0
        ? roundToTwoDecimals(
            (counts.alignmentCount / counts.applicableCount) * this.scale
          )
        : 0;
