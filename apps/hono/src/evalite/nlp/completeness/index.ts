...
export class CompletenessMetric extends Metric {
  measure(input: string, output: string): CompletenessMetricResult {
    ...
    const inputDoc = nlp(inputTrimmed);
    const outputDoc = nlp(outputTrimmed);
    ...
    return {
      ...
      info: {
        inputElements,
        outputElements,
        missingElements: inputElements.filter(
          (e) => !outputElements.includes(e)
        ),
        elementCounts: {
